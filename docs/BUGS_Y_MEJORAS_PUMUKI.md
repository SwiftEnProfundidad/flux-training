...
| PUM-009 | 2026-03-05 | Bug | SDD/Evidence | `pumuki sdd evidence` vuelve a generar artefacto incompleto (`version: \"1.0\"` o sin `slices[]`) en consumer, aunque en core se marcó cerrado. | Alta (reabre bloqueo `TDD_BDD_EVIDENCE_INVALID` en commits frontend reales). | Secuencia reproducida: `pnpm exec pumuki sdd evidence --scenario-id=docs/validation/features/p3_t1_web_shell_dashboard ...` -> `pumuki watch --once --stage=PRE_COMMIT --scope=staged` => `Invalid input: expected array, received undefined`. | Publicar versión consumer que incorpore fix de core y añadir test de regresión en scaffold para garantizar `version: \"1\"` + `slices[]` siempre. | 🚧 En construccion |
| PUM-010 | 2026-03-05 | Mejora | Skills/Gate DX | El stack de skills coverage no queda estable entre iteraciones; sin `adapter/bootstrap/policy reconcile` vuelve a bloquear commits frontend aunque el código sea válido. | Alta (fricción recurrente y tiempo extra en cada commit atómico). | Repro 2 iteraciones seguidas: `pnpm exec pumuki watch --once --stage=PRE_COMMIT --scope=staged --json` => `SKILLS_PLATFORM_COVERAGE_INCOMPLETE_HIGH` + `SKILLS_SCOPE_COMPLIANCE_INCOMPLETE_HIGH`; se desbloquea tras `adapter install + bootstrap + policy reconcile`. | Persistir activación de bundles `frontend-guidelines` por repo/sesión y autovalidar antes del hook para evitar bootstrap manual repetitivo. | ⏳ Pendiente |
| PUM-011 | 2026-03-05 | Bug | Watch/Consumer parity | `pumuki watch --once --json` en consumer no incluye `lastTick.changedFiles[]` ni `lastTick.evaluatedFiles[]`, pese a estar indicado como fix en core. | Media (limita trazabilidad por commit y auditoría de alcance real evaluado). | Repro local: `pnpm exec pumuki watch --once --stage=PRE_COMMIT --scope=staged --json` -> `lastTick` sin `changedFiles/evaluatedFiles`. | Publicar en npm la versión con payload extendido y añadir test de contrato JSON en consumer para evitar drift entre core y paquete distribuido. | ⏳ Pendiente |
...
- `PUM-010` detectado en runtime de hooks:
  - el gate de skills coverage vuelve a estado incompleto entre iteraciones (`frontend-guidelines` no activo/evaluado) hasta repetir `adapter/bootstrap/policy reconcile`.
  - impacto directo: bloquea commits frontend legítimos y rompe fluidez de commits atómicos.
