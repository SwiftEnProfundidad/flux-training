  ...
  implements TrainingGateway, NutritionGateway, ProgressGateway, ObservabilityGateway, OfflineSyncGateway
{
  ...
  async listObservabilitySummary(userId: string): Promise<ObservabilitySummary> {
    const events = this.analytics.filter((event) => event.userId === userId);
    const reports = this.crashes.filter((report) => report.userId === userId);
    return {
      userId,
      generatedAt: "2026-03-02T20:20:00.000Z",
